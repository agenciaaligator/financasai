...
// Versão interna (servidor-a-servidor) do google-calendar-sync: chamada pelo
// google-calendar-webhook quando o Google avisa que algo mudou. Não tem JWT
// de usuário (é o próprio backend chamando), então autentica por segredo
// compartilhado, no mesmo padrão do whatsapp-usage-increment.
//
// Header: x-webhook-secret: GOOGLE_CALENDAR_SYNC_SECRET
// Body: { user_id: uuid }
...
Deno.serve(async (req) => {
  ...
  try {
    const expected = Deno.env.get("GOOGLE_CALENDAR_SYNC_SECRET");
    ...
    const { user_id: userId } = await req.json().catch(() => ({ user_id: undefined }));
    ...
    for (const ev of items) {
      ...
      if (upsertError) {
        console.error("[sync-internal] upsert failed for event", ev.id, upsertError);
        continue;
      }
      synced++;
    }
    ...
    console.error("google-calendar-sync-internal error:", msg);
