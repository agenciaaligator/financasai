      ...
      <div className="bg-primary/5 border-b">
        <div className="container mx-auto px-4 py-10 md:py-14">
          <div className="flex items-center gap-4 max-w-3xl mx-auto">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold">{t('legal.privacy.title')}</h1>
              <p className="text-sm text-muted-foreground mt-1">{t('legal.privacy.lastUpdated')}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="bg-card rounded-2xl border shadow-sm p-6 md:p-10 space-y-8">
              {sectionKeys.map((key) => (
                <section key={key} id={`section-${key}`} className="scroll-mt-24">
                  <h2 className="text-lg font-semibold mb-3">{t(`legal.privacy.sections.${key}.title`)}</h2>
                  <p className="text-muted-foreground leading-relaxed">{t(`legal.privacy.sections.${key}.content`)}</p>
                </section>
              ))}
            </div>
          </main>
