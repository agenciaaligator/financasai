  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-secondary/20">
      ...
      <main className="container mx-auto px-4 py-12 max-w-3xl">
        ...
        <p className="text-sm text-muted-foreground mb-6">{t('legal.terms.lastUpdated')}</p>

        <div className="prose prose-sm max-w-none space-y-6 text-foreground">
          {sectionKeys.map((key) => (
            <section key={key}>
              <h2 className="text-xl font-semibold mb-3">{t(`legal.terms.sections.${key}.title`)}</h2>
              <p className="text-muted-foreground">{t(`legal.terms.sections.${key}.content`)}</p>
            </section>
          ))}
        </div>
      </main>
